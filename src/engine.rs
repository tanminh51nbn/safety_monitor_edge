...
impl AIEngine {
    ...
    pub fn process_frame(
        ...
        let target_w = self.input_w as f32;
        let target_h = self.input_h as f32;
        ...
        let resized_img =
            image::imageops::resize(img, new_unpad_w, new_unpad_h, FilterType::Triangle);

        // Tính khoảng cách viền đen cần bù
        let pad_w = (self.input_w - new_unpad_w) / 2;
        let pad_h = (self.input_h - new_unpad_h) / 2;

        // 2.2. Khởi tạo mảng Tensor (1, 3, H, W) có giá trị nền là Màu xám (YOLO chuẩn cần là RGB 114)
